...
FPTPlay final docs use a **single source-of-truth markdown spec per sub-feature**:

```text
features/final-docs/<Large-Feature>/<Sub-Feature>/product/<feature-name>.md
```

`product/<feature-name>.md` is the implementation contract. It combines Product, UX, integration expectations, state/behavior rules, and error handling, following the structure of the PD source doc when one exists.

Do not auto-create `<feature-name>-mockup.html`. Create a mockup/prototype only when the user explicitly asks.

Do not create or maintain legacy split contracts (`product/functional-specification.md`, `api/technical-contract.md`, `design/design-contract.md`). The legacy split convention is removed system-wide.
...
Keep all surface-level UI details in `8.4 Surface Details by Surface` inside `product/<feature-name>.md`.
